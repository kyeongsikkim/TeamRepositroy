...
function requestThermistorSensorData() {
	...
	ws = new WebSocket("ws://" + location.host + "/SensingCarWebProject/websocket/thermistorsensor");
...
function requestGasSensorData() {
	...
	ws = new WebSocket("ws://" + location.host + "/SensingCarWebProject/websocket/gassensor");
	ws.onmessage = function(event) {
		var data = JSON.parse(event.data);
		var series = sensorChart.series[0];
		var shift = series.data.length > 20;
...
function requestPhotoresistorSensorData() {
	...
	ws = new WebSocket("ws://" + location.host + "/SensingCarWebProject/websocket/photoresistorsensor");
	ws.onmessage = function(event) {
		var data = JSON.parse(event.data);
		var series = sensorChart.series[0];
		var shift = series.data.length > 20;
...
function requestUltrasonicSensorData() {
	...
	ws = new WebSocket("ws://" + location.host + "/SensingCarWebProject/websocket/ultrasonicsensor");
	ws.onmessage = function(event) {
		var data = JSON.parse(event.data);
		var series = sensorChart.series[0];
		var shift = series.data.length > 20;
...
function requestTrackingSensorData() {
	...
	ws = new WebSocket("ws://" + location.host + "/SensingCarWebProject/websocket/trackingsensor");
	ws.onmessage = function(event) {
		var data = JSON.parse(event.data);
		var series = sensorChart.series[0];
		var shift = series.data.length > 20;
